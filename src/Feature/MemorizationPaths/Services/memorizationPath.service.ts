...
const getMemorizationPaths = async (params: MemorizationPathQueryParams) => {
  ...
  const response = await apiClient.get(`${MEMORIZATION_PATHS}`, {
    params: queryParams,
  });
  return response.data as ResponseMemorzationPath;
};
...
const getMemorizationPathDetails = async (
  ...
  const response = await axios.get(`${BASE_URL}/${MEMORIZATION_PATHS}/${id}`);
  return response.data;
};
