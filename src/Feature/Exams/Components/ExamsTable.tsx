      ...
      <Table>
        <TableHeader className='bg-[#CB997E]'>
          <TableRow>
            ...
            <TableHead className='text-start text-white'>تاريخ الانتهاء</TableHead>
          ...
          {listOfExams.length > 0 &&
            listOfExams.map((exam) => (
              <TableRow key={exam.id} className='font-semibold bg-white'>
                ...
                <TableCell>
                    <Badge variant={exam.examCategory === 'Manhaj' ? 'default' : 'secondary'}>
                        {exam.examCategory === 'Manhaj' ? 'منهج' : 'حفظ'}
                    </Badge>
                </TableCell>
                <TableCell>{exam.examDegree}</TableCell>
                <TableCell>{new Date(exam.startDateTime).toLocaleDateString()}</TableCell>
                <TableCell>{new Date(exam.endDateTime).toLocaleDateString()}</TableCell>
