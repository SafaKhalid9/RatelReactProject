...
export default function EditExam() {
  ...
  useEffect(() => {
    if (examData?.data) {
      ...
      form.reset({
        halaqaIds: exam.halaqaIds.map((id: number, i: number) => ({
          value: id.toString(),
          label: exam.halaqaNames[i],
        })),

        manhajIds: exam.manhajIds.map((id: number, i: number) => ({
          value: id.toString(),
          label: exam.manhajNames[i],
        })),

        teacherIds: exam.teacherIds.map((id: number, i: number) => ({
          value: id.toString(),
          label: exam.teacherNames[i],
        })),
        // Note: fullName isn't in ExamType, so we might need a better label or fetch teacher details
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select
                    ...
                    key={form.watch("examTypeId") || "examTypeId"}
                  >
                    <FormControl>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select
                    ...
                    value={field.value}
                    defaultValue={field.value}
                    key={form.watch("examCategory") || "examCategory"}
                  >
                    <FormControl>
                      <SelectTrigger className="w-full">
                    ...
                    <SelectContent>
                      <SelectItem value="Manhaj">منهج</SelectItem>
