...
import { useManhajs, useMemorizationPaths, useTeachers } from '../Services/halaqa.service';
...
const HalaqaForm = ({ mode, defaultValues, onSubmit, isLoading }: HalaqaFormProps) => {
  const [formData, setFormData] = useState<HalaqaFormData>(defaultValues || INITIAL_STATE);
  const [errors, setErrors] = useState<Partial<Record<keyof HalaqaFormData, string>>>({});
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-y-4 pb-5 px-10">
      ...
      <div className="flex justify-between gap-x-10">
        <div className="w-1/2">
          ...
          <Select 
            ...
            onValueChange={(val) => setFormData({...formData, status: val as HalaqaStatus})}
          >
            <SelectTrigger>
      ...
      <div className="flex justify-between gap-x-10">
        <div className="w-1/2">
          ...
          <Select 
            ...
            onValueChange={(val) => setFormData({...formData, teacherID: parseInt(val)})}
          ...
          >
           ...
           {errors.teacherID && <span className="text-red-500 text-sm">{errors.teacherID}</span>}
        </div>
      </div>
     ...
     <div className="flex justify-between gap-x-10">
        <div className="w-1/2">
          <span className="text-xl font-semibold mb-2 block">Paths (Hold Ctrl to select multiple mock)</span>
          ...
          <div className="border p-4 rounded-md space-y-2 h-40 overflow-y-auto">
            {loadingPaths ? <div className="text-sm text-gray-500">Loading Paths...</div> : 
             paths?.map(path => (
              <div key={path.id} className="flex items-center gap-2">
                <input 
                  type="checkbox" 
                  checked={formData.selectedPathIds.includes(path.id)}
                  onChange={() => handleMultiSelectChange('selectedPathIds', path.id.toString())}
                />
                <span>{path.name}</span>
              </div>
            ))}
        ...
        <div className="w-1/2">
          ...
          <div className="border p-4 rounded-md space-y-2 h-40 overflow-y-auto">
            {loadingManhajs ? <div className="text-sm text-gray-500">Loading Manhajs...</div> :
             manhajs?.map(m => (
              <div key={m.id} className="flex items-center gap-2">
                <input 
                  type="checkbox" 
                  checked={formData.selectedManhajIds.includes(m.id)}
                  onChange={() => handleMultiSelectChange('selectedManhajIds', m.id.toString())}
                />
                <span>{m.name}</span>
              </div>
            ))}
