...
import { AppreciationGrade, AppreciationGradeFormData } from '../Types/appreciationGrade.types';
...
// API Functions
const getAppreciationGrades = async (page: number = 1, pageSize: number = 10) => {
...
const getAppreciationGradeById = async (id: number): Promise<AppreciationGrade> => {
    const response = await axios.get(`${BASE_URL}/${APPRECIATION_GRADES_URL}/${id}`);
    return response.data;
};

const addAppreciationGrade = async (data: AppreciationGradeFormData) => {
    const response = await axios.post(`${BASE_URL}/${APPRECIATION_GRADES_URL}`, data);
    return response.data;
};

const updateAppreciationGrade = async ({ id, data }: { id: number; data: AppreciationGradeFormData }) => {
    const response = await axios.put(`${BASE_URL}/${APPRECIATION_GRADES_URL}/${id}`, data);
    return response.data;
};

const deleteAppreciationGrade = async (id: number) => {
    const response = await axios.delete(`${BASE_URL}/${APPRECIATION_GRADES_URL}/${id}`);
    return response.data;
};

// Hooks
export const useAppreciationGrades = (page: number = 1, pageSize: number = 10) => {
    return useQuery({
