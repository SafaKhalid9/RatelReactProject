...
import { AppreciationGrade } from '../Types/appreciationGrade.types';
...
const AppreciationGradesTable: React.FC<AppreciationGradesTableProps> = ({ grades }) => {
    return (
        <div className="rounded-md border">
            <Table>
                <TableHeader className="bg-[#CB997E]">
                    <TableRow>
                        <TableHead className="text-start text-white">رمز التقدير (Appreciation ID)</TableHead>
                ...
                <TableBody>
                    {grades.length === 0 ? (
                        ...
                        grades.map((grade) => (
                            <TableRow key={grade.id} className="font-semibold bg-white even:bg-slate-50">
                                <TableCell>
                                    <div className="flex gap-2 items-center">
                                        <Badge variant="outline">{grade.appreciationId}</Badge>
                                        {grade.appreciationName && <span>{grade.appreciationName}</span>}
                                    </div>
                                </TableCell>
