...
import type { AppreciationGradeFormData } from "../Types/appreciationGrade.types";
...
const AppreciationGradeForm: React.FC<AppreciationGradeFormProps> = ({
  defaultValues,
  onSubmit,
  isLoading = false,
  mode,
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<AppreciationGradeFormData>({
    defaultValues: {
      appreciationId: 0,
      minimumScore: 0,
      maximumScore: 100,
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          />
          {errors.appreciationId && (
            <span className="text-red-500 text-sm">
              {errors.appreciationId.message}
            </span>
          )}
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Controller
            ...
            rules={{
              ...
              max: { value: 100, message: "يجب أن يكون أقل من أو يساوي 100" },
              validate: (value) =>
                Number(value) > Number(minimumScore) ||
                "يجب أن يكون الحد الأعلى أكبر من الحد الأدنى",
            }}
            render={({ field }) => (
              <Input
                {...field}
